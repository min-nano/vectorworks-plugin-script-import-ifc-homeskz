...
def build_story_commands(ifc_file: ifcopenshell.file) -> list[StoryCommand]:
    ...
    for i, (elevation, beam_offset) in enumerate(stories):
        ...
        # 含む場合に持つ(下屋根は母屋を持たなくても屋根版=垂木・野地板があるため、母屋
        # とは別に屋根版の有無で判定する)。スタックは
        # 横架材天端/軒高 ← 母屋 ← 垂木 ← 野地板(上ほど上段)で、垂木を母屋の直上に、
        # 野地板を垂木の直上に積む(母屋が無ければ横架材天端/軒高の直上)。高さはいずれも
        # 横架材天端(最上階は軒高)に揃える(実描画の Z は母屋・垂木・野地板の要素が持つ)。
        # 最上階はさらに野地板の直上に母屋伏図用の小屋束記号レイヤを積む。
