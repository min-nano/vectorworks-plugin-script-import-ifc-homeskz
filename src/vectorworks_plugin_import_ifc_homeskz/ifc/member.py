...
傾斜梁)を登り梁に使うと、端部が勾配ぶん(せい/2·sinθ)軒側へずれて受ける材との間に
隙間ができるため、直切りの幾何(XY ずらし無し)で端部の XY を鉛直端面の位置に合わせる。
なお**登り梁の最終的な高さ・勾配・端部位置は ``ifc/noboribari.py`` の
``correct_noboribari`` が後処理で整える**(端部を受ける材/柱の面まで詰め、天端中央線を
真上の屋根版=垂木下面の平面へスナップして勾配・高さを屋根勾配に合わせる)。ここで求める
直切りの高さ ``せい/(2·cosθ)`` は屋根版が見つからないときのフォールバックになる。
