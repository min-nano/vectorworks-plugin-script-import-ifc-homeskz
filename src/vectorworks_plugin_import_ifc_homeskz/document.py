...
スキーマ (version 33):

    {
        ...
        "roofs": [
            {
                # 野地板(屋根の下地合板)。屋根版(IfcSlab の屋根面)1 面ごとに
                # 屋根ツール(vs.BeginRoof)で単勾配の屋根オブジェクトを作図する。
                # 厚みは 12mm 固定(要件: IFC に情報が無ければ 12mm)。
                "layer": "R-野地板",       # 配置先デザインレイヤ名(既存のみ・なければスキップ)
                "class": "04構造-02木造-06耐力面材-03屋根",  # 割り当てるクラス名
                # 屋根の水平投影外形(平面ポリゴンの頂点列、mm・センタリング済み)。
                # BeginRoof の 2D テンプレート(床ツールと同じ Begin.../EndGroup パターン)。
                "boundary": [[x1, y1], [x2, y2], [x3, y3], [x4, y4]],
                # 軒(最も低い辺)に沿う屋根軸の 2 点(BeginRoof の p1/p2、
                # mm・センタリング済み)。屋根面はこの軸から upslope 方向へ立ち上がる。
                "axis_start": [x1, y1],
                "axis_end": [x2, y2],
                # 棟(高い)側を指す upslope 定義点(BeginRoof の upslope、
                # mm・センタリング済み)。軸から棟側へ向かう単位方向の点。
                "upslope": [x3, y3],
                # 勾配(BeginRoof の riseDistance/runDistance)。slope = rise/run。
                # 屋根面の単位法線の水平成分(dh)と鉛直成分(nz)から rise=dh・run=nz。
                "rise": 400.0,
                "run": 1000.0,
                "thickness": 12.0,        # 野地板厚 (mm、固定値 12)
                # 軒(屋根軸)の天端 Z の絶対値 (mm)。BeginRoof は軸を Z=0 で作るため、
                # 描画フェーズは作成後 Move3D(0,0,elevation) で軒の高さへ移動する。
                "elevation": 6060.0
            }
        ],
        "columns": [
        ...
        "slabs": [
            {
                ...
                # 空(省略可)。連続する地中梁を 1 本の 3D パスにまとめ、台形断面をパスに
                # 沿って掃引した「パスに沿った押し出し」でモデリングする(屈曲部も統合)。
                "modifiers": [
                    {
                        # 台形断面(u=進行方向左向き・v=鉛直方向の 2D 頂点列)。
                        "profile": [[0.0, 0.0], [-150.0, 0.0],
                                    [-290.0, 140.0], [0.0, 140.0]],
                        # 掃引パス(断面基準点 (0,0) が辿る 3D 折れ線)。屈曲部を含む
                        # 場合は 3 点以上。XY はセンタリング済み・z は絶対値(梁下端)。
                        "path": [[100.0, 200.0, -240.0],
                                 [1160.0, 200.0, -240.0]]
                    }
                ]
            }
        ...
        "joints": [
            {
                ...
                # (受ける材)に取り付く場合、または柱の側面に取り付く場合にだけ、
                # その端部に置く(取り付かない自由端には置かない)。
        ...
        "column_marks": [
            {
                # 各 span 柱レイヤ(``{from}to{to}-柱``)に重ねて、その柱レイヤ自身の
                # 柱・小屋束を記号化する「断面記号」。柱束伏図記号 PIO(姉妹プロジェクト
                # vectorworks-plugin-column-under-mark)を断面スタイルで配置し、実断面に
                # 合わせた ×(柱)・/(小屋束)を描く。対象クラスは絞らず(空=全クラス)
                # span 柱レイヤの構造用途 4/5 の構造材すべてを記号化する。
                "layer": "1to2-柱",       # PIO を配置するデザインレイヤ名(span 柱レイヤ自身)
                "class": "01作図-01線-02実線-01極細線",  # 断面記号の作図クラス(極細実線)
                "target_layer": "1to2-柱",  # 同じ span 柱レイヤを検索対象にする
                "target_class": "",       # クラスを絞らない(全対象)
                "size": 300.0,            # 実断面が取れないときのフォールバックサイズ
                "style": "断面",          # 記号スタイル(断面記号=実断面の ×・/)
                "symbol": "",             # 断面記号ではシンボルを使わない
                # PIO の挿入点(センタリング済み)。記号は検索した柱のワールド位置に
                # 描かれ挿入点には依存しないため原点でよい。
                "position": [0.0, 0.0]
            },
            {
                # 各 span 柱レイヤの柱・小屋束を、その span の ``to``(span 上側の数値)を
                # プレフィックスにした専用レイヤ ``{to}-柱伏図記号`` に平面記号(シンボル)
                # として描く「伏図記号」。span レイヤは単一種別なので、柱の span は
                # 柱伏図記号、小屋束の span は 束伏図記号 を単一の MarkSymbol に渡す
                # (柱用と小屋束用は別々のオブジェクト)。各伏図は切断位置の直下
                # (to < 切断)の伏図記号レイヤだけを表示する(ifc/sheet.py)。
                "layer": "2-柱伏図記号",   # PIO を配置するデザインレイヤ名({to}-柱伏図記号)
                "class": "01作図-04記号-04構造-一般",  # PIO 本体(記号)を作図するクラス
                "target_layer": "1to2-柱",  # PIO が柱を検索する対象レイヤ(span 柱レイヤ)
                "target_class": "",       # クラスを絞らない(全対象)
                "size": 300.0,            # 記号サイズ(シンボル指定時は依存しない)
                "style": "平面",          # 記号スタイル(平面記号=伏図記号)
                "symbol": "柱伏図記号",    # この span の種別のシンボル(PIO の MarkSymbol)
                "position": [0.0, 0.0]
            }
        ],
        "legends": [
        ...
        "rebars": [
            {
                ...
                "class": "04構造-01基礎-09鉄筋",  # PIO 本体の描画クラス(PIO は全図形をこのクラスで描く)
...
# 'class' キーが Python の予約語のため functional 構文で定義する(GridCommand と同様)
RoofCommand = TypedDict('RoofCommand', {
    'layer': str,
    'class': str,
...
"""野地板(屋根の下地合板)を屋根ツール(BeginRoof)で描画する命令。

屋根版(IfcSlab の屋根面)1 面ごとに単勾配の屋根オブジェクトを作図する。
``boundary`` は屋根の水平投影外形(BeginRoof の 2D テンプレート、床ツールと同じ
Begin.../EndGroup パターンで描く)。``axis_start``/``axis_end`` は軒(最も低い辺)に
沿う屋根軸(BeginRoof の p1/p2)、``upslope`` は棟(高い)側を指す upslope 定義点。
``rise``/``run`` は勾配(slope=rise/run。屋根面の単位法線の水平成分 dh と鉛直成分 nz)。
``thickness`` は野地板厚で、要件により 12mm 固定(IFC に情報が無ければ 12mm)。
``elevation`` は軒(屋根軸)の天端 Z の絶対値で、BeginRoof が軸を Z=0 で作るため
描画フェーズは作成後 Move3D で軒の高さへ移動する。``class`` は割り当てるクラス名
(耐力面材-屋根)。配置先レイヤ(``layer``)は垂木レイヤの直上に独立させた n-野地板。
"""
...
class ModifierCommand(TypedDict):
    """底盤スラブに噛み合わせる地中梁(台形断面)のモディファイア。パス押し出し。

    地中梁は台形断面のため単一のスラブオブジェクトでは描けない。底盤(基礎底盤)の
    コンクリートに 3D ソリッド(モディファイア)として噛み合わせて実形状を表す
    (VW のスラブは path 図形の profile 群にモディファイアソリッドを持たせて削り取る)。

    **連続する地中梁を 1 本の 3D パスにまとめ、台形断面をそのパスに沿って掃引した
    「パスに沿った押し出し」(``CreateExtrudeAlongPath``)でモデリングする**(屈曲部=
    コーナーで向きの変わる地中梁も、断面が一致すれば 1 本のパスに統合する)。

    ``path`` は掃引パス(断面基準点=``profile`` の (0,0) が辿る 3D 折れ線)の
    頂点列 ``[[x, y, z], ...]``(2 点以上、XY はセンタリング済み・z は絶対値=梁下端の
    Z)。``profile`` は掃引する台形断面の 2D 頂点列 ``[[u, v], ...]`` で、
    u=進行方向左向きの水平軸・v=鉛直方向(v=0 が断面基準点の高さ=梁下端)。断面の
    向き(u 軸の水平方位)はパス始端の接線(``path[0]→path[1]``)の左向きに揃える。

    描画フェーズはパス頂点から degree-1 の NURBS カーブ(掃引パス)を、始端接線の
    左向き ``u``・鉛直 ``v`` で断面を始端に置いた 3D ポリゴン(プロファイル)を作り、
    ``CreateExtrudeAlongPath(パス, プロファイル)`` で押し出しソリッドにする
    (``vw/footing.py`` 参照)。
    """
...
modifiers は底盤に噛み合う地中梁(台形断面)のモディファイア(``ModifierCommand``)
のリスト。地中梁は台形断面のため単一スラブでは描けず、底盤コンクリートにモディファイア
として噛み合わせる(要件)。連続する地中梁を 1 本の 3D パスにまとめ、台形断面をパスに
沿って掃引した押し出しソリッドにする(屈曲部も統合)。無ければ空リスト。描画フェーズは
modifiers があるスラブの削り取り(``SetCustomObjectProfileGroup``)と可視ソリッドを
このパス押し出しソリッドから作る。
"""
...
``elevation`` は床下端の絶対 Z で、IFC の床版ソリッドの最下端(床位置)を尊重
した値(ストーリ高さ + ローカル最下端 Z)。``bound`` は床下端の高さ基準で、
標準の床高=配置先ストーリの横架材天端レベル(= ストーリ高さ + 横架材天端
オフセット)にバインドし、``offset`` に床下端と横架材天端の差分(基準高さからの
高低差)を格納する。床下端が横架材天端ちょうどの床は offset 0、段差のある床
(スキップフロア)や床高の異なる床は offset がずれる。``class`` は割り当てる
構造クラス名(床板)。
"""
...
class JointCommand(TypedDict):
    ...
    横架材の端部が別の横架材(受ける材)に取り付く場合、または柱の側面に取り付く
    場合にだけ、その端部に仕口シンボルを置く(取り付かない自由端には置かない)。
    layer は配置先デザイン
...
"""柱束伏図記号 PIO(断面記号・伏図記号)を配置する命令。

カスタム PIO「柱束伏図記号」を ``layer`` に置く。PIO はリセット時に
``target_layer`` の構造用途 4/5 の構造材を検索し、``style`` の記号スタイルに
従って柱・小屋束の記号を各位置に描く。柱・小屋束は span(``{from}to{to}-柱``)
ごとのレイヤに配置され、各 span レイヤは単一種別(柱のみ=構造用途 4、または
小屋束のみ=構造用途 5)なので、**柱用と小屋束用は別々のオブジェクト**(別々の
span レイヤ・別々のシンボル)として配置される。用途は 2 種類:

- **断面記号**(``style`` = ``断面``): 各 span 柱レイヤ(``n to m-柱``)に重ねて、
  そのレイヤ自身の柱・小屋束を実断面に合わせた記号(柱×・小屋束/)で表す。``layer``・
  ``target_layer`` はともに span 柱レイヤ自身、``target_class`` は空(全クラス)、
  ``symbol`` は空(× / / を描く)。
- **伏図記号**(``style`` = ``平面``): 各 span 柱レイヤの柱・小屋束を、その span の
  ``to``(span 上側の数値)をプレフィックスにした専用レイヤ ``{to}-柱伏図記号`` に
  平面記号(シンボル)として描く。``layer`` はその ``{to}-柱伏図記号`` レイヤ、
  ``target_layer`` は span 柱レイヤ自身、``target_class`` は空、``symbol`` はその
  span の種別に応じたシンボル(柱の span なら ``柱伏図記号``、小屋束の span なら
  ``束伏図記号``)。span レイヤが単一種別のため 1 レイヤ 1 種別・1 シンボルで
  正しく描き分けられる(PIO は変更せず既存の単一 MarkSymbol を使う)。

``class`` は PIO 本体(=描かれる記号)を作図するクラス名(``vs.SetClass`` で
設定する)で、検索対象クラス ``target_class`` とは別物。伏図記号は柱・束の記号の
作図クラス、断面記号は極細実線の作図クラスを使う。``target_class`` は検索対象クラス
(空=全クラス)、``size`` は記号サイズ(mm、PIO の MarkSize に渡す。断面記号では
実断面が取れないときのフォールバック)、``style`` は記号スタイル(``平面`` / ``断面``。
PIO の MarkStyle に渡す)、``symbol`` は伏図記号で配置するシンボル名(PIO の
MarkSymbol に渡す。断面記号では空文字)、``position`` は PIO の挿入点(記号は検索した
...
def _validate_roof(index: int, command: Any) -> None:
    ...
    for key in ('axis_start', 'axis_end', 'upslope'):
        _require(_is_point(command.get(key)),
                 f'{where}.{key} は [x, y] の数値ペアである必要があります')
    for key in ('rise', 'run', 'thickness', 'elevation'):
        _require(_is_number(command.get(key)),
                 f'{where}.{key} は数値である必要があります')
...
def _validate_column_mark(index: int, command: Any) -> None:
    ...
    _require(isinstance(command.get('style'), str) and command['style'],
             f'{where}.style は非空文字列である必要があります')
    # symbol は伏図記号で配置するシンボル名。断面記号では空文字。空文字を許容する
    # ため非空チェックはしない。
    _require(isinstance(command.get('symbol'), str),
             f'{where}.symbol は文字列である必要があります')
    _require(_is_point(command.get('position')),
             f'{where}.position は [x, y] の数値ペアである必要があります')
...
def validate_document(document: Any) -> Document:
    ...
    for i, command in enumerate(document['roofs']):
        _validate_roof(i, command)
