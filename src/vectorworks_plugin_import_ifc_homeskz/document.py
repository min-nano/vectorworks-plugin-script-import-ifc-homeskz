...
- **下階柱記号**(``style`` = ``平面``): 各階の伏図に直下階(N-1)の柱を記号化する。
  ``layer`` は横架材天端(最上階は軒高)レイヤの直上の ``n-下階柱``、
  ``target_layer`` は直下階の ``n-柱`` レイヤ。柱(×)と束(○)を別オブジェクトに
  分けて描画設定を独立調整するため、1 階分につき ``target_class`` を管柱クラスに
  絞った PIO と小屋束クラス(直下階に架かる下屋根の小屋束)に絞った PIO の 2 つを
  置く(通し柱には記号を付けない)。
- **小屋束記号**(``style`` = ``平面``): 母屋伏図に最上階(屋根)の小屋束を記号化する。
  ``layer`` は母屋レイヤの直上の ``R-小屋束``、``target_layer`` は屋根の ``R-柱``
  レイヤ、``target_class`` は小屋束クラス(小屋束だけに絞り、柱の下階柱記号とは
  別オブジェクトにする)。
