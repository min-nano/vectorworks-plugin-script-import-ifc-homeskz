...
スキーマ (version 33):

    {
        ...
        "column_marks": [
            {
                # 各階の伏図(床伏図・小屋伏図)に直下階(N-1)の管柱を記号化する
                # 「下階柱記号」。カスタム PIO「柱束伏図記号」(姉妹プロジェクト
                ...
                # 検索対象クラスを管柱クラスに絞り、管柱(×)だけを記号化する(通し柱には
                # 付けない)。下屋根(下屋)の小屋束は小屋組の要素であり母屋伏図に表示する
                # ため下階柱記号には含めず、その階の小屋束記号(下記)が母屋伏図に記号化する
                # (含めると床伏図に下階の小屋束が誤表示され、小屋束記号と重複もするため)。
            ...
            {
                # 母屋伏図に屋根の小屋束を記号化する「小屋束記号」。屋根版を持つ階
                # (最上階の主屋根・中間階の下屋根)ごとに、その階の柱レイヤ(n-柱)を
                # 検索対象にし、検索対象クラスを小屋束クラスに絞ることで小屋束(○)だけを
                # 記号化する(柱の下階柱記号とはクラスで分けた別オブジェクト)。
                "layer": "R-小屋束",      # PIO を配置するデザインレイヤ名(野地板の直上)
                "class": "01作図-04記号-04構造-一般",  # PIO 本体(記号)を作図するクラス
            ...
            {
                # 各階の柱レイヤ(n-柱)に重ねて、その柱レイヤ自身の柱・小屋束を
                # 記号化する「断面記号」。柱束伏図記号 PIO を断面スタイルで配置し、
                # 実断面に合わせた ×(柱)・/(小屋束)を描く。対象クラスは絞らず
                # (空=全クラス)柱レイヤの構造用途 4/5 の構造材すべてを記号化する。
                "layer": "1-柱",          # PIO を配置するデザインレイヤ名(柱レイヤ自身)
                "class": "01作図-01線-02実線-01極細線",  # 断面記号の作図クラス(極細実線)
                "target_layer": "1-柱",   # 同じ柱レイヤを検索対象にする
...
"""柱束伏図記号 PIO(下階柱記号・小屋束記号・断面記号)を配置する命令。

カスタム PIO「柱束伏図記号」を ``layer`` に置く。PIO はリセット時に
``target_layer`` の構造用途 4/5 の構造材を検索し、``style`` の記号スタイルに
従って柱・小屋束の記号を各位置に描く。用途は 3 種類:

- **下階柱記号**(``style`` = ``平面``): 各階の伏図(床伏図・小屋伏図)に直下階
  (N-1)の管柱を記号化する。``layer`` は横架材天端(最上階は軒高)レイヤの直上の
  ``n-下階柱``、``target_layer`` は直下階の ``n-柱`` レイヤ。``target_class`` を管柱
  クラスに絞り管柱(×)だけを記号化する(通し柱には付けない)。下屋根(下屋)の小屋束は
  小屋組の要素であり母屋伏図に表示するため下階柱記号には含めず、その階の小屋束記号
  (下記)が母屋伏図に記号化する(含めると床伏図に下階の小屋束が誤表示され、小屋束記号と
  重複もするため)。
- **小屋束記号**(``style`` = ``平面``): 母屋伏図に屋根の小屋束を記号化する。屋根版を
  持つ階(最上階の主屋根・中間階の下屋根)ごとに 1 つ。``layer`` は野地板レイヤの直上の
  ``n-小屋束``、``target_layer`` はその階の ``n-柱`` レイヤ、``target_class`` は小屋束
  クラス(小屋束だけに絞り、柱の下階柱記号とは別オブジェクトにする)。
- **断面記号**(``style`` = ``断面``): 各階の柱レイヤ(``n-柱``)に重ねて、その柱
  レイヤ自身の柱・小屋束を実断面に合わせた記号(柱×・小屋束/)で表す。``layer``・
  ``target_layer`` はともに柱レイヤ自身、``target_class`` は空(全クラス)。

``class`` は PIO 本体(=描かれる記号)を作図するクラス名(``vs.SetClass`` で
設定する)で、検索対象クラス ``target_class`` とは別物。下階柱記号・小屋束記号は
柱・束の伏図記号の作図クラス、断面記号は極細実線の作図クラスを使う。
``target_class`` は検索対象クラス(空=全クラス)、``size`` は記号サイズ
(mm、PIO の MarkSize に渡す。断面記号では実断面が取れないときのフォールバック)、
``style`` は記号スタイル(``平面`` / ``断面``。PIO の MarkStyle に渡す)、
