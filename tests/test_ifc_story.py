...
class TestBuildStoryCommands:
    def test_shed_dormer_without_moya_gets_taruki_but_no_moya(self) -> None:
        ...
        # 2 階は屋根版を持つが母屋を持たないため、垂木・野地板レベルのみ(母屋レベルなし)。
        # 垂木・野地板は横架材天端の直上に積む。柱は span レイヤに載るが、このテストの
        # 柱は断面が無く span レイヤが生成されないため 柱 レベルは付かない。
    ...
    def test_intermediate_story_with_moya_gets_moya_level(self) -> None:
        ...
        # (このテストの柱は断面が無く span レイヤが生成されないため 柱 レベルは付かない)
        assert commands[1]['name'] == '2階'
    ...
    def test_builds_commands_for_three_stories(self) -> None:
        ...
        # このテストの柱は断面が無く span レイヤが生成されないため 柱 レベルは付かない。
        # 下階柱・小屋束(伏図記号)レベルは span 方式で廃止した。
        assert commands == [
            {
    ...
    def test_single_story_treated_as_roof(self) -> None:
        ...
        assert level_types == ['野地板', '垂木', '母屋', '軒高']

    def test_span_column_levels_stacked_on_top(self) -> None:
        """柱の span レイヤは base ストーリの levels 先頭(スタック最上段)に (from, to)
        昇順で積まれ、level type はレイヤ名と同じ。"""
        ifc = ifcopenshell.file()
        make_storey(ifc, '1FL', 473.0, [('IfcColumn', -48.0)])
        make_storey(ifc, '2FL', 3273.0, [('IfcSlab', -36.0)])
        make_storey(ifc, 'RFL', 5973.0)
        column_layers = {0: ['1to2-柱'], 1: ['2to2.5-柱', '2to3-柱']}

        commands = build_story_commands(ifc, column_layers)

        assert commands[0]['levels'][0] == {
            'type': '1to2-柱', 'offset': -48.0, 'layer': '1to2-柱'}
        assert commands[1]['levels'][:2] == [
            {'type': '2to2.5-柱', 'offset': -36.0, 'layer': '2to2.5-柱'},
            {'type': '2to3-柱', 'offset': -36.0, 'layer': '2to3-柱'},
        ]
