...
class TestExecuteWalls:
    ...
    def test_updates_styled_objects_after_placing(self) -> None:
        """全配置後に UpdateStyledObjects で壁スタイルの描画属性(クラス→テクスチャ
        等)を反映する。SetWallStyle は関連付けまでで描画属性をプッシュしないため、
        これを呼ばないとテクスチャ等が反映されない(柱・梁と同じ規約。#56/#57)。
        壁を何枚配置してもスタイル単位で 1 回。
        """
        vs_mock = _make_vs_mock({'F-立上り'})
        vw_footing = _load(vs_mock)
        ...
        # 個別設定(高さバインド等)を終えた全配置後に呼ぶ
        methods = [c[0] for c in vs_mock.mock_calls]
        assert methods.index('UpdateStyledObjects') > methods.index('SetWallStyle')
