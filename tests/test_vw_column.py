...
def make_column_command(layer: str = '1-横架材天端', column_type: str = '管柱',
                        position: tuple[float, float] = (0.0, 0.0),
                        width: float = 105.0, depth: float = 105.0,
                        height: float = 2844.0, elevation: float = 426.0) -> ColumnCommand:
    return {
        'layer': layer,
...
class TestExecuteColumns:
    ...
    def test_switches_to_command_layer(self) -> None:
        vs_mock = _make_vs_mock(existing_layers={'1-横架材天端', 'R-軒高'})
        _run_execute_columns(vs_mock, [
            make_column_command(layer='1-横架材天端'),
            make_column_command(layer='R-軒高'),
        ])
