...
このスクリプトは実行のたびに GitHub の `main` ブランチの最新コミットを確認し、インストール済みのコミットと異なれば、VectorWorks 設定フォルダ内の Python Externals フォルダへ pip で自動的に更新インストールしてからプラグイン本体を実行します（`main` は常にテスト済みのため、バージョン番号ではなくコミット SHA で判定します）。パッケージが未インストールの場合（初回起動時）は依存ライブラリも含めて自動的に新規インストールします。インターネットに接続できないなど更新を確認できない場合は、アップグレードをスキップしてインストール済みのバージョンをそのまま実行します。
