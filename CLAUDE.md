...
- ストーリレベル: 一般階は `FL`(0) と `横架材天端`(負値)、最上階は `軒高`(0)。加えて全階に柱配置用の `柱` レベルを持つ（オフセットは横架材天端と同じ＝最上階は軒高の 0）。柱は `n-柱` レイヤに梁と同じ構造材ツールで配置する。`levels` の並び順がそのままデザインレイヤのスタック順（ナビゲーションのレイヤ表示順）になるため、`柱` レベルを先頭に置き、`n-柱` レイヤが `n-FL`（最上階は `R-軒高`）レイヤの直上に来るようにする。
