...
- 命令セットの型は `document.py` の `TypedDict`（`Document` / `StoryCommand` / `GridCommand` / `MemberCommand` / `ColumnCommand` / `WallCommand` / `SlabCommand` / `AnchorBoltCommand` / `FireBraceCommand` / `SheetCommand` / `ViewportCommand` / `TagCommand` / `LevelCommand` / `StoryBoundCommand`）を使う。`GridCommand` / `MemberCommand` / `ColumnCommand` / `WallCommand` / `SlabCommand` は `class` キー（構造クラス名）が予約語のため functional 構文で定義している（`AnchorBoltCommand` / `FireBraceCommand` / `SheetCommand` / `ViewportCommand` は `class` キーを持たないため通常の `class` 構文で定義する）。スキーマ変更時は `TypedDict` 定義・docstring・`validate_document()` を同時に更新すること。
...
5. **描画（フェーズ2）** — `vw.execute_document(document)` が検証後、ストーリ → 通り芯 → 構造材 → 柱 → 立上り（壁）→ 底盤/地中梁（スラブ）→ アンカーボルト（シンボル）→ 火打（シンボル）の順で描画し、デザインレイヤのスタック順を `reorder_story_layers` で整えたあと、最後にシートレイヤ（ビューポート）と横架材の断面寸法データタグ（ビューポート注釈）を描画して実行数を返す。シート・タグはデザインレイヤと横架材オブジェクトを参照するため、レイヤ生成・並べ替え・構造材描画の完了後に描画する。構造材描画（`execute_members`）は配置した横架材ハンドルをインデックス付きで記録し、シート描画（`execute_sheets`）がタグをこのハンドルに関連付ける。
...
- 解析・柱梁伏図（`ifc/sheet.py` の `build_floor_framing_sheet_commands`）: **ストーリ（`collect_stories`）1 つにつき伏図 1 枚**。シートレイヤ番号・図番は基礎伏図（`1`）に続けて `2` から順（最下階＝`2`）。タイトル・図面タイトルは最下階から `1階床伏図`・`2階床伏図`・…、最上階は `小屋伏図`（`floor_plan_title`）。表示レイヤは各階の**横架材**（一般階 `n-横架材天端`・最上階 `n-軒高`）・**柱**（`n-柱`）・**通り芯**（`共通`）を基本とし、**最上階以外は床**（`n-FL`）も表示する。**最下階（1 階）には基礎がある場合のみアンカーボルト**（`F-アンカーボルト`）も加える。レイヤの並びは 横架材・柱・（アンカーボルト・床）・通り芯 の順。ストーリが無ければ空リストを返す。
