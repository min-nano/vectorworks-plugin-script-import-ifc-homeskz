...
- 最上階以外の階では、`IfcRelContainedInSpatialStructure` を辿って `IfcColumn` のローカル配置 Z 座標（0 または負値、例: `-48.0`, `-36.0`）を集め、その**最頻値**を**横架材天端**の相対オフセットとして使用します。柱は横架材の上に載るため最頻値が横架材天端に一致し、ポーチ柱・デッキ柱などより深い位置から立つ外れ値の柱（最小値だとこれを拾ってしまう）に基準を引きずられない。最頻値が同数のときは FL に近い方（大きい方）を採用し決定的な結果にする。柱が無い階のみ `IfcSlab` で代用する（柱がある階で床版を同列に数えると、バルコニー等の床下げ床版に基準を引きずられる）。正の Z は対象外。
...
- ストーリレベル: 一般階は `FL`(0) と `横架材天端`(0 または負値)、最上階は `軒高`(0) のみ。
