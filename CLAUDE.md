...
src/
    vectorworks_plugin_import_ifc_homeskz/   # pip インストール可能なパッケージ本体
        ...
        vw/                   # フェーズ2: VectorWorks 描画 (vs 依存)
            ...
            column.py         # column 命令 → 柱・間柱オブジェクト
...
5. **描画（フェーズ2）** — `vw.execute_document(document)` が検証後、ストーリ → 通り芯 → 構造材 → 柱の順で描画し実行数を返す。
...
- 解析: `IfcColumn` を走査し、配置・断面寸法（`IfcRectangleProfileDef` の `XDim`/`YDim`）・柱高さ（`IfcExtrudedAreaSolid.Depth`）から column 命令を組み立てる。XY 座標は通り芯と同じ中心オフセットで補正。柱は各階の柱レイヤ `n-柱`（命令の `layer`）に配置し、柱・間柱ツールの伏図記号は伏図レイヤ `n-柱(伏図)`（命令の `plan_layer`）に描く。高さ基準のレベルタイプ（横架材天端／軒高）は配置レイヤとは別に引き続き参照する。
- 高さ（上下端）: 固定の柱高さフィールドではなく**ストーリレベル基準**で上下端をバインドし、階高変更に追従できるようにする（柱・間柱ツールの「高さ基準(下/上)」）。各 column 命令は `bottom_bound`／`top_bound`（`StoryBound`: `story`=boundStory・`level`=レベルタイプ名・`offset`=オフセット mm）を持つ。
  - 高さ基準(下): 当該階（`story=0`）の横架材天端（最上階の柱は `軒高`）。`offset` = `柱底部の絶対 Z − 当該階基準レベルの絶対 Z`。
  - 高さ基準(上): 上階（`story=1`）の横架材天端、ただし上階が最上階なら `軒高`。`offset` = `柱頂部の絶対 Z − 上階基準レベルの絶対 Z`。上階が無い最上階の柱は当該階（`story=0`）の `軒高` を上下端とも基準にする。
  - 命令内の `height`（押し出し深さ）・`elevation`（柱底部の絶対 Z）はフォールバック描画と `Height` フィールド表示にのみ使う。
- 種別: `IfcColumn.ObjectType`（`None` または `STANDCOLUMN`）を木造BIM 柱・間柱ツールの種別名に変換する（`None`→`管柱`、`STANDCOLUMN`→`小屋束`、未知の値は `管柱`）。ツールの種別ドロップダウンの有効値は `管柱`/`通し柱`/`間柱`/`小屋束`/`吊木`、断面は `矩形`/`円形`/`シンボル定義`。断面は現状すべて `矩形`。
- 柱頭・柱脚金物: ホームズ君 IFC では柱頭・柱脚金物が柱と同じストーリに含まれる `IfcMechanicalFastener`（柱頭/柱脚付近の立方体）として表現される。名前に `柱頭金物`／`柱脚金物` を含む金物を、柱と**同じ平面座標**（XY を丸めたキー）で柱に対応付け、金物の型 `IfcMechanicalFastenerType` の名前（例: `柱頭金物:(ろ)`・`柱頭金物:C12`）のコロン以降を仕様文字列として `top_hardware`／`bottom_hardware` に格納する（該当金物が無ければ空文字）。型は IfcRelDefinesByType 経由で辿り、逆方向属性名がスキーマで異なる（IFC2X3=`IsDefinedBy`／IFC4=`IsTypedBy`）ため両方を走査する。
- 描画: 木造BIM 柱・間柱ツール `vs.CreateCustomObject('AAPillarS', 0, 0, 0)` をローカル原点に生成し `Move3D` で XY 位置へ移動。`CreateCustomObject`／`SetRField` にはローカライズ名（`柱・間柱`）ではなく**ユニバーサル名**（`AAPillarS`）を渡す。ローカライズ名を渡すと VW がその名前のパラメトリックオブジェクト型を新規定義しようとし、プラグインが登録済みの同名レコードフォーマット（`柱・間柱`）と衝突して「同じ名前のレコードフォーマットがすでに存在しています」エラーになる（構造材 `StructuralMember`・通り芯 `GridAxis` と同様にユニバーサル名で統一）。上下端は `vs.SetObjectStoryBound(obj, boundID, 2, boundStory, levelType, offset)` でストーリレベルにバインドする（`boundID` は上端=0・下端=1、`boundType=2` は Story 基準）。さらに伏図記号フィールド（`伏図記号を表示`=`True`・`伏図レイヤ`=命令の `plan_layer`）と柱頭・柱脚金物フィールド（`TopHard`／`BtmHard`=命令の `top_hardware`／`bottom_hardware`）を設定する。`TopHard`／`BtmHard` は柱・間柱ツールが `ResetObject` 時に継手条件から再計算する出力フィールドのため、`ResetObject` の**後**に書き込み以降は再リセットしない（前に設定すると空に上書きされる。取り込んだ柱には継手ジオメトリが無いため空に再計算される）。各階の `n-柱(伏図)` レイヤに伏図記号を描く。配置先レイヤが存在しない命令はスキップし、プラグインが使えない場合は断面の矩形にフォールバック。
...
- デザインレイヤ名は `1-FL`, `1-横架材天端`, `1-柱`, `1-柱(伏図)`, ..., `R-軒高`, `R-柱`, `R-柱(伏図)`（接頭辞はストーリ suffix と一致）。
- ストーリレベル: 一般階は `FL`(0) と `横架材天端`(負値)、最上階は `軒高`(0)。加えて全階に柱配置用の `柱` と伏図用の `柱(伏図)` レベルを持つ（オフセットは横架材天端と同じ＝最上階は軒高の 0）。柱は `n-柱` レイヤに配置し、柱・間柱ツールの伏図記号を `n-柱(伏図)` レイヤに描く。
