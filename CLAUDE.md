...
- 解析: `IfcBeam` / `IfcMember` を走査し、配置・断面寸法・材種から member 命令（座標は通り芯と同じ中心オフセットで補正、構造材 ID は `{幅}×{背} - {材種}`）を組み立てる。一般階は `横架材天端` レイヤ、最上階は `軒高` レイヤを指定。
  - 配置高さ（`elevation`）は各横架材自身のローカル配置 Z を用い `ストーリ高さ + ローカル Z`（絶対値）とする。レイヤの横架材天端の基準高さに固定しないため、基準高さにない梁（段差のある梁等）も正しい高さに描画される。Z を取得できない梁のみレイヤ基準高さ（`ストーリ高さ + resolve_beam_top_offset`、最上階はストーリ高さ）にフォールバックする。
  - 食い込み調整（`resolve_member_interferences`）: 命令組み立て後、横架材同士が T 字状に食い込んでいる箇所（甲乙梁の取り合い等）の端部を相手梁の面まで詰めて干渉を解消する。ある梁の端点が別の梁の矩形「内部」（端部近傍＝L 字仕口は除外）に入り、配置レイヤが一致し Z 範囲（`[天端 - 背, 天端]`）が重なる場合のみ対象。相手梁（通し材）の形状は変えず食い込む側だけを短くする。平行（同一直線上の継ぎ手）・Z が離れた段差梁・幅内に達していない端部は対象外。判定は入力時点のジオメトリに対して行い命令の並び順に依存しない。
...
- 解析: `IfcColumn` を走査し、配置・断面寸法（`IfcRectangleProfileDef` の `XDim`/`YDim`）・柱高さ（`IfcExtrudedAreaSolid.Depth`）から column 命令を組み立てる。XY 座標は通り芯と同じ中心オフセットで補正。横架材と同じレイヤ割り当て規則で一般階は `横架材天端`、最上階（屋根の小屋束等）は `軒高` レイヤを指定。
...
- 描画: 木造BIM 柱・間柱ツール `vs.CreateCustomObject('柱・間柱', 0, 0, 0)` をローカル原点に生成し `Move3D` で XY 位置へ移動。上下端は `vs.SetObjectStoryBound(obj, boundID, 2, boundStory, levelType, offset)` でストーリレベルにバインドする（`boundID` は上端=0・下端=1、`boundType=2` は Story 基準）。配置先レイヤが存在しない命令はスキップし、プラグインが使えない場合は断面の矩形にフォールバック。
