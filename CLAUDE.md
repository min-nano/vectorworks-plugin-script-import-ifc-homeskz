...
命令セットのスキーマ（version・stories/grids/members/columns/walls/slabs/anchor_bolts/sheets 各命令の形式）は `document.py` の docstring に定義されている。スキーマを変更するときは `DOCUMENT_VERSION` の互換性に注意し、`validate_document()` とテストも併せて更新すること。`run()` は両フェーズの間で `json.dumps`/`json.loads` を通すため、命令セットに直列化不能なオブジェクト（ifcopenshell エンティティや vs ハンドル等）を入れてはならない。
...
src/
    vectorworks_plugin_import_ifc_homeskz/   # pip インストール可能なパッケージ本体
        ...
        vw/                   # フェーズ2: VectorWorks 描画 (vs 依存)
            ...
            sheet.py          # sheet 命令 → シートレイヤ・ビューポート
...
5. **描画（フェーズ2）** — `vw.execute_document(document)` が検証後、ストーリ → 通り芯 → 構造材 → 柱 → 立上り（壁）→ 底盤/地中梁（スラブ）→ アンカーボルト（シンボル）の順で描画し、デザインレイヤのスタック順を `reorder_story_layers` で整えたあと、最後にシートレイヤ（ビューポート）を描画して実行数を返す。シートはデザインレイヤを参照するため、レイヤ生成・並べ替えの完了後に描画する。
