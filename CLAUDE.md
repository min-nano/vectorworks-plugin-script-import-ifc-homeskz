...
- 解析（`build_fire_brace_commands`）: 横架材と同じく FL ストーリを走査し、各ストーリの `ContainsElements` から `Name` が `火打` 始まりの `IfcBeam`/`IfcMember` を対象にする。配置先レイヤは属する階の横架材レイヤ。座標は通り芯・横架材と同じグリッド中心オフセットで補正する。
  ...
  - **回転角（`angle`）**: ユーザー指定により火打の向きに合わせて回転する。基準点（内角）から火打本体の重心へ向かう方向（＝内角の二等分方向）を採る（`_angle`）。4 方向の直交コーナーだけでなく非 90 度コーナー（斜め壁等）にも追従する。**シンボルの基準姿勢（0 度での向き）は VectorWorks 上で最終確認する**（描画フェーズは他要素と同じく VW 上で検証する方針）。
