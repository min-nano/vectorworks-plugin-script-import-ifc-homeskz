...
コード修正を実施する際の以下のプロセスに従う:

1. **PR作成の判断基準**:
   - 実装がレビュー可能なレベルに達した時点で PR を作成する
   - 迷いや未確定事項がある場合（変更方針をユーザーに確認中など）は、PR 作成を保留する

2. **PR 作成後の対応**:
   - PR を作成したら CI 結果とレビューコメントを監視する
   - レビューコメントや CI 失敗に対しては、内容を確認してから対応を判断する
   - 簡単な修正は自動的に追加コミットで対応してよいが、大きな変更や判断が必要な指摘は、対応前にユーザーに確認する
