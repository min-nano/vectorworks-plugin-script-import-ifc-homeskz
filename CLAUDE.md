...
モデルデータ（ストーリ・通り芯・基礎など）を取り込んだ後に、特定のデザインレイヤ群を表示するビューポートを配置した**シートレイヤ**を作成する。IFC からシート構成を読み取るのではなく、取り込んだ要素の有無からどのシートを作るべきかを解析フェーズが判断する。現状は **基礎伏図** の 1 枚のみ。

- 解析（`ifc/sheet.py` の `build_sheet_commands`）: **基礎要素が存在する場合にだけ**基礎伏図の sheet 命令を返す（`footing.has_foundation` で判定。基礎が無ければ表示すべき `F-底盤`・`F-立上り`・`F-アンカーボルト` レイヤが生成されずビューポートが空になるため、空リストを返す）。sheet 命令は シートレイヤ番号 `1`・タイトル `基礎伏図`、ビューポートの図面タイトル `基礎伏図`・図番 `1`、表示レイヤ `F-底盤`・`F-立上り`・`F-アンカーボルト`・`共通`（底盤・立上り・アンカーボルト・通り芯の順）を持つ。
