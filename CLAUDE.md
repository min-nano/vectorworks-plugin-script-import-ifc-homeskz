...
- 描画: 木造BIM 柱・間柱ツール `vs.CreateCustomObject('柱・間柱', 0, 0, 0)` をローカル原点に生成し `Move3D` で XY 位置へ移動。上下端は `vs.SetObjectStoryBound(obj, boundID, 2, boundStory, levelType, offset)` でストーリレベルにバインドする（`boundID` は上端=0・下端=1、`boundType=2` は Story 基準）。さらに伏図記号フィールド（`伏図記号を表示`=`True`・`伏図レイヤ`=命令の `plan_layer`）と柱頭・柱脚金物フィールド（`TopHard`／`BtmHard`=命令の `top_hardware`／`bottom_hardware`。柱・間柱ツール AAPillarS の実フィールド名で、ドロップダウン候補に無いカスタム仕様も `SetRField` でそのまま格納される）を設定し、各階の `n-柱(伏図)` レイヤに伏図記号を描く。配置先レイヤが存在しない命令はスキップし、プラグインが使えない場合は断面の矩形にフォールバック。
