...
- 解析: `IfcColumn` を走査し、配置・断面寸法（`IfcRectangleProfileDef` の `XDim`/`YDim`）・柱高さ（`IfcExtrudedAreaSolid.Depth`）から column 命令を組み立てる。XY 座標は通り芯と同じ中心オフセットで補正。配置 Z 高さは `ストーリ高さ + 柱のローカル配置 Z`（負値）の絶対値。横架材と同じレイヤ割り当て規則で一般階は `横架材天端`、最上階（屋根の小屋束等）は `軒高` レイヤを指定。
...
- 描画: 木造BIM 柱・間柱ツール `vs.CreateCustomObject('柱・間柱', 0, 0, 0)` をローカル原点に生成し `Move3D` で絶対位置へ移動。配置先レイヤが存在しない命令はスキップし、プラグインが使えない場合は断面の矩形にフォールバック。
