...
- 描画（`vw/sheet.py` の `execute_sheets`）: シートレイヤを `vs.CreateLayer(title, 2)`（種別 2 = プレゼンテーション＝シートレイヤ）で作成し（同名があれば再利用）、シートレイヤ番号・タイトルを `vs.SetObjectVariableString`（selector `158`＝番号 / `159`＝タイトル）で設定する。その上に `vs.CreateVP(sheetLayer)` でビューポートを作り、**表示レイヤを絞り込む**（`configure_viewport_layers`: 全デザインレイヤを `FLayer`→`NextLayer` で辿ってビューポートの親シートレイヤ以外を一旦非表示にし＝`SetVPLayerVisibility(vp, layer, 2)`、命令の `layers` に挙げたレイヤだけを名前で引いて表示に戻す＝`SetVPLayerVisibility(vp, layer, 0)`）。図面タイトル・図番を `vs.SetObjectVariableString`（selector `1032`＝図面タイトル / `1034`＝図番）で設定し、`vs.UpdateVP` で描画を更新する。ビューポートが生成できない場合はシートレイヤだけ作って設定はスキップする。
- 実行順: シートはデザインレイヤを参照するため、`execute_document` はストーリ・通り芯・構造材・基礎の描画とレイヤ並べ替え（`reorder_story_layers`）を**すべて終えた後**に `execute_sheets` を呼ぶ。
- **オブジェクト変数 selector（`SetVPLayerVisibility` の表示種別 0/2 を含む）は VectorWorks 公式のオブジェクト変数一覧に基づく**。シートレイヤの番号=`158`・タイトル=`159` は確認済み。ビューポートの図面タイトル=`1032`・図番=`1034` は VW での実挙動により最終確認する（描画フェーズは他の要素と同じく VectorWorks 上で検証する方針）。selector 値は `vw/sheet.py` 冒頭の名前付き定数（`_OV_SHEET_NUMBER` 等）に集約しており、変更が必要な場合はそこを直す。
