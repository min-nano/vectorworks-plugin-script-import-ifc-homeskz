...
- 命令セットの型は `document.py` の `TypedDict`（`Document` / `StoryCommand` / `GridCommand` / `MemberCommand` / `ColumnCommand` / `WallCommand` / `SlabCommand` / `AnchorBoltCommand` / `SheetCommand` / `ViewportCommand` / `TagCommand` / `LevelCommand` / `StoryBoundCommand`）を使う。`GridCommand` / `MemberCommand` / `ColumnCommand` / `WallCommand` / `SlabCommand` は `class` キー（構造クラス名）が予約語のため functional 構文で定義している（`AnchorBoltCommand` / `SheetCommand` / `ViewportCommand` は `class` キーを持たないため通常の `class` 構文で定義する）。スキーマ変更時は `TypedDict` 定義・docstring・`validate_document()` を同時に更新すること。
...
- 解析（`ifc/tag.py` の `build_tag_commands`）: **横架材の member 命令から**タグの配置を算出する（IFC を直接見ない）。横架材 1 本につき tag 命令 1 つ。`build_document` は横架材命令を一度だけ組み立てて `members` と `build_tag_commands(members)` の両方に渡す（tag 命令の `member_index` が `members` の並びと一致する必要があるため）。tag 命令は `style`（=`断面寸法`）・`layer`（横架材の配置レイヤ）・`member_index`（`members` 内インデックス）・`position`（挿入位置）・`angle`（文字角度）を持つ。
  ...
  - **配置側（上または左）**: 軸に直交する 2 候補（±90 度回転）から「上または左」を向く側を選ぶ（`_offset_side`）。y が大きい（上）方を優先し、y が同等な南北向きの材は x が小さい（左）方を選ぶ。これにより東西材は上、南北材は左にタグが並ぶ。
  - **位置**: 横架材の中央から、選んだ直交方向へ `断面幅/2 + 余白`（`_TAG_OFFSET_MARGIN`、既定 100mm）だけオフセットした点。余白は VW 上で見え方を調整する際に変える。座標は他要素と同じグリッド中心オフセット済み（member 命令の座標をそのまま使う）。
