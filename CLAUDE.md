...
- 最上階以外の階では、`IfcRelContainedInSpatialStructure` を辿って `IfcColumn`・`IfcSlab` のローカル配置 Z 座標（例: `-48.0`, `-36.0`, `0.0`）を集め、その**最頻値**を**横架材天端**の相対オフセットとして使用します。柱はその階の横架材天端に立つため、柱底部が最も集中する高さが横架材天端になります。最小値ではなく最頻値を使うのは、一部の柱が仕口などで他より深く配置されている場合（例: 53 本が `-10` のところ 1 本だけ `-200`）に、最小値だと多数派の柱が横架材天端から浮いてしまうためです。候補が同数のときは最小値を選び、エンティティ列挙順に依存しない決定的な結果にします。
